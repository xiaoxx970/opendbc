...
def create_acc_accel_control(packer, bus, CP, CCP, acc_type, acc_enabled, upper_jerk, lower_jerk, upper_control_limit, lower_control_limit,
  ...
  values = {
    ...
    # 2 = engine start mandatory, 1 = auto stop prohibited, 0 = auto stop allowed.
    # Only allow auto stop after the hold confirmation has been filtered by standstill.
    "ACC_StartStopp_Info":        2 if (acc_enabled and starting) else (1 if acc_enabled and not esp_hold else 0),
