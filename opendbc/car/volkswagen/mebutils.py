  ...
  def update(self, enabled, override, distance, has_lead, accel, critical_state):
    ...
    if not enabled:
      ...
      if has_lead:
        ...
        filter_gain_dist_change = np.interp(abs(distance_change), self.FILTER_GAIN_DISTANCE_CHANGE,
                                            [self.jerk_limit_min, self.FILTER_GAIN_MAX]) # gain by distance change
...
class LongControlLimit:
  ...
  def update(self, enabled: bool, speed: float, set_speed: float, distance: float, has_lead: bool, critical_state: bool):
    # control limits by distance are used to improve comfort while ensuring precise car reaction if necessary
    ...
    if not enabled or critical_state: # force most precise accel command execution
      ...
      # Reset the filter for a new lead and correctly force the current state on the next iteration.
      ...
      upper_limit_dist_change = np.interp(-min(0, distance_change), self.LIMIT_DISTANCE_CHANGE_UP,
                                          [self.UPPER_LIMIT_MAX, self.LIMIT_MIN]) # limit by distance change up
      ...
      # Set speed decrease requested by the user or speed overshoot, including the HUD-to-real-speed difference.
      ...
      lower_limit_dist_change = np.interp(max(0, distance_change), self.LIMIT_DISTANCE_CHANGE_DOWN,
                                          [self.LOWER_LIMIT_MAX, self.LIMIT_MIN]) # limit by distance change down
...
def sigmoid_curvature_boost_meb(kappa: float, v_ego: float, kappa_thresh: float = 0.0) -> float:
  # compensate non linear behavior: boost low curvatures
  # this is either a model issue (nerfing low curvatures) or a specific steering rack behavior
