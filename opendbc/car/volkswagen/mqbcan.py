...
class DynamicChecksumConfig(TypedDict):
  length: int
  magic: list[int]
...
def volkswagen_mqb_meb_dyn_len_checksum(address: int, sig, d: bytearray, entry: DynamicChecksumConfig | None = None) -> int:
  const = None
  if entry:
    length = entry["length"]
    d = d[:length]
...
VOLKSWAGEN_MQB_MEB_GEN2_CONSTANTS: dict[int, DynamicChecksumConfig] = {
  ...
  # The corresponding calculation checks checksum correctness by itself and falls back if necessary.
