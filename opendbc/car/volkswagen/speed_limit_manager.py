...
class SpeedLimitManager:
  def __init__(self, car_params, speed_limit_max_kph=SPEED_SUGGESTED_MAX_HIGHWAY_GER_KPH, predicative=False,
               predicative_speed_limit=False, predicative_curve=False):
    self.CP = car_params
  ...
  def get_speed_limit_predicative(self):
    v_limit_output = self.v_limit_psd_next if (self.predicative and self.v_limit_psd_next != NOT_SET and
                                               self.v_limit_psd_next < self.v_limit_output_last) else NOT_SET
  ...
  def _build_predicative_segments(self, psd_04, psd_06, raining, time_car):
    ...
    if (psd_04["PSD_ADAS_Qualitaet"] == 1 and
      ...
      if seg:
        ...
        seg = self.predicative_segments[segment_id]
        seg["Curve_Speed_Begin"] = self._calculate_curve_speed(seg["Curvature_Begin"])
        seg["Curve_Speed_End"] = self._calculate_curve_speed(seg["Curvature_End"])

    # Schritt 2: Alte Segment bereinigen
  ...
  def _hit_linear_profile(self, v0_ms, a, total_dist_m, L_m, v_b_kmh, v_e_kmh):
    ...
    # Flaches Profil => Standard-Bremsweg gegen v_b am Begin
    if abs(r) < 1e-9:
        if v0_ms <= v_b:
            return None, None
  ...
  def _get_speed_limit_psd_next(self, current_speed_ms):
    ...
    if best_result["limit"] != float('inf'):
      ...
      if (now - self.v_limit_psd_next_last_timestamp <= self.v_limit_psd_next_decay_time and
          self.v_limit_output_last > self.v_limit_psd_next_last and not self.v_limit_changed):
