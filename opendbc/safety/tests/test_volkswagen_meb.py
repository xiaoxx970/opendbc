...
# sunnypilot: MQBevo shares the MEB safety hooks (alias)
...
# sunnypilot: MQBevo Long variant
class TestVolkswagenMqbEvoLongSafety(TestVolkswagenMebLongSafety):
  def setUp(self):
    self.packer = CANPackerSafety("vw_mqbevo_generated")
    self.safety = libsafety_py.libsafety
    self.safety.set_safety_hooks(CarParams.SafetyModel.volkswagenMqbEvo, VolkswagenSafetyFlags.LONG_CONTROL)
    self.safety.init_tests()
